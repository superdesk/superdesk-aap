...
class BroadcastAutoPublishTestCase(AAPTestCase):
    ...
    def test_broadcast_copy_state_published(self, internal_dest):
        item = self.get_item()
        broadcast_auto_publish(item, desk="a", stage="b")
        self.assertNotIn("line-#121#", item["body_html"])
        self.assertEqual(
            [{"name": "Broadcast Script", "qcode": "Broadcast Script"}], item["genre"]
        )
    ...
    def test_broadcast_copy_legal(self, internal_dest):
        ...
        broadcast_auto_publish(item, desk=1, stage=2)
    ...
    def test_broadcast_if_body_html_is_short(self, internal_dest):
        ...
        item["type"] = "text"
        item["body_html"] = (
            "<p> </p><p>The first case of a contaminated strawberry has been found in the "
